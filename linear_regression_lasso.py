...
OUTPUT_PATH = ''
OUTPUT_METRICS = os.path.join(OUTPUT_PATH, 'lasso_metrics.json')
OUTPUT_PREDICTIONS = os.path.join(OUTPUT_PATH, 'lasso_predictions.xlsx')
RUN_BOOTSTRAP = True
...
def bootstrap_r2_ci(X_train, y_train, X_test, y_test, n_iter = 1000):
    """Performs bootstrap for lasso model.
    ...
    for i in range(n_iter):
        ...
        try: 
            pipeline_boot = Pipeline([
                ('scaler', RobustScaler()),
                ('regressor', LassoCV(cv=5, max_iter=1000, random_state=SEED))
            ])
...
try:
    ...
    if RUN_BOOTSTRAP:
        ...
        joblib.dump(model, os.path.join(OUTPUT_PATH, "lasso_model.joblib"))
